(function(){
  ...
  Snix.accessor = function(expr, context, vars){
    ...
    try{
      ...
      console.info("expr", readerExpr, "keys", readerKeys)
      console.error(err);
    }
  ...
  Snix.refresh = function(context, el){
    ...
    // $("[data-bind]", el).removeAttr("data-refresh");

    // $("[data-bind]:not([data-guids])", el).attr("data-refresh", "true");
    // $("[data-bind][data-guids]", el).filter(function(){
    //   var guidsArr = $(this).attr("data-guids").split(" ");
    //   return _.intersection(guidsArr, onlyGuids).length > 0;
