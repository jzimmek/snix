...
Snix.val = function(initial, opts){
  ...
  f.set = function(newValue){
    if(this.isDifferentValue(newValue)){
      ...
      this.value = newValue;
      _.invoke(this.changeListener, "apply", this, [newValue, oldValue]);
    }
  }
  ...
  f.on = function(event, fun, context){
    this[event+"Listener"].push(_.bind(fun, context||this));
    return this;
  };

  f.one = function(event, fun, context){
    var f = function(){
      fun.apply(context||this, arguments);
      this[event+"Listener"] = _.without(this[event+"Listener"], f);
    };
    this[event+"Listener"].push(f);
    return this;
  };
...
Snix.Record = function(baseUrl, attributes){
  ...
  var events = ["create", "save", "duplicate", "destroy"];
  ...
  r.prototype.on = function(event, fun, bind){
    this[event+"Listener"].push(_.bind(fun, bind||this));
    return this;
  };
  ...
  r.prototype.save = function(){
    ...
    var ajax = $.ajax({
      ...
      _.invoke(this.saveListener, "call");
      _.invoke(r.saveListener, "call");
  ...
  r.prototype.duplicate = function(){
    ...
    var ajax = $.ajax({
      ...
      _.invoke(this.duplicateListener, "call");
      _.invoke(r.duplicateListener, "call");
      Snix.refresh(this.context);
    }).error(function(err){ alert("err:"+err); });
  ...
  r.prototype.destroy = function(){
    ...
    var ajax = $.ajax({
      ...
      _.invoke(this.destroyListener, "call");
      _.invoke(r.destroyListener, "call");
      Snix.refresh(this.context);
    }).error(function(err){ alert("err:"+err); });
  ...
  r.prototype.create = function(){
    ...
    var ajax = $.ajax({
      ...
      _.invoke(this.createListener, "call");
      _.invoke(r.createListener, "call");
      Snix.refresh(this.context);
    }).error(function(err){ alert("err:"+err); });
...
Snix.Filter.prototype.filter = function(arr){
  return _.select(Snix.unwrap(arr), function(e){
    ...
    for(var key in this.fields){
      ...
      if(type == "in" && this[key]().length > 0)          valid = valid && _.include(this[key](), value);
      else if(type == "like" && this[key]().length > 0)   valid = valid && value && value.toString().toLowerCase().indexOf(this[key]().toLowerCase()) >= 0;
    }
